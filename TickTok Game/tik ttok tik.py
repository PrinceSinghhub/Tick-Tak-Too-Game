...
def WithPc():
    ...
    # root1.iconbitmap("tr.png")

    image1 = Image.open('tr.png')
    ...
    def insertLatter(latter , position):
        ...
        if board[position] == ' ':
            ...
            if(checkDraw()):
                ...
                if latter == 'X':
                    MSG.showinfo('AI Tic Tac Toe', 'Player 1 is winner')
                    playerX = playerX + 1
                    playerX_var.set('Player 1 : '+str(playerX))

                    disable_all_buttons()
                else:
                    MSG.showinfo('AI Tic Tac Toe', 'Player 2 is winner')
                    playerO = playerO + 1
                    playerO_var.set('Player 2 : '+str(playerO))
                    disable_all_buttons()
        else:
